...
```bash
cd ~/seglab
bun run models       # one-time: vendors ~23 MB of ORT wasm + verifies the built weights
...
onnxruntime-web under `lib/` and verifies the built weights under `models/`
(both gitignored); the app prefers those and only falls back to the pinned CDN
for the runtime. The weights themselves are produced by `scripts/export-*.py`,
not fetched — no hub publishes the exact artifacts this app runs. After it has run, import,
...
Nothing downloads at startup. Unvendored, the first image import warms the mask lane
...
can't be spoofed upward — a usable WebGPU adapter (so the lane runs ~0.5 GB on
...
| Setting | lite value |
| --- | --- |
| Interaction proxy | **≤ 1024 px long edge** — every import path (JPEG/PNG/WebP/AVIF/HEIF/TIFF/GIF/BMP/RAW preview or on-device develop/paste/drop). 1024 is SAM 2.1's own internal encode edge, so the model sees its exact native frame |
...
frames**. The mask lane (and magic-wand / colour / brush) work on the bounded
...
bounded proxy/export settings only; segmentation remains single-lane and
...
`256 × 256` is SAM 2.1's internal decoder-mask resolution, not the image
...
A proxy larger than 1024 px cannot invent model detail because the encoder runs
...
- `js/sam21-lane.js` / `sam21-host.js` / `sam21-client.js` / `sam21-adapter.js` — SAM 2.1 inference; one shared ORT session per origin; encode-once/decode-per-click
- `js/sam-client.js` — the single main-thread facade over that lane; `sam-core.js` — pure mask math (no model runtime)
